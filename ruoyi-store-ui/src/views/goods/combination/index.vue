<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['goods:combination:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['goods:combination:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['goods:combination:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['goods:combination:export']"
        >导出</el-button>
      </el-col>
    </el-row>
    ...
    <el-table v-loading="loading" :data="combinationList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="商品组合名称" align="center" prop="name" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="商品组合名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入商品组合名称" />
        </el-form-item>
...
<script>
import { listCombination, getCombination, delCombination, addCombination, updateCombination, exportCombination } from "@/api/goods/combination";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getCombination(id).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除商品组合编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delCombination(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有商品组合数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportCombination(queryParams);
        }).then(response => {
